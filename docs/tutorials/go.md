...
```sh
go get github.com/hellarpro/tenderhellar@latest
```
...
execute `tenderhellar init validator`. But before we do that, we will need to install
...
```bash
rm -rf /tmp/example
TMHOME="/tmp/example" tenderhellar init validator
