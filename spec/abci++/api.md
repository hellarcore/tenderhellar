...
| data | [bytes](#bytes) |  | Result bytes, if any (arbitrary data, not interpreted by Tenderhellar). |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| type | [tendermint.types.VoteExtensionType](#tendermint-types-VoteExtensionType) |  | Vote extension type can be either DEFAULT, THRESHOLD_RECOVER or THRESHOLD_RECOVER_RAW. The Tenderhellar supports only THRESHOLD_RECOVER and THRESHOLD_RECOVER_RAW at this moment. |
...
Tenderhellar will use SHA256 checksum of `sign_request_id` when generating quorum signatures of THRESHOLD_RECOVER_RAW vote extensions. It MUST NOT be set for any other vote extension types. |
| sign_request_id | [bytes](#bytes) | optional | If not set, Tenderhellar will generate it based on height and round.
...
- The application can choose to refetch chunks and/or ban P2P peers as appropriate.
  Tenderhellar will not do this unless instructed by the application.
...
- When all chunks have been accepted, Tenderhellar will make an ABCI Info call to verify that LastBlockAppHash
...
- If Tenderhellar is unable to retrieve the next chunk after some time (e.g. because no suitable peers are available),
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| chunk_id | [bytes](#bytes) |  | The chunk index, starting from 0. Tenderhellar applies chunks sequentially. |
...
- `ResponseExtendVote.vote_extensions` is optional information that, if present, will be signed by Tenderhellar and
...
- `ResponseExtendVote.vote_extensions` will only be attached to a non-`nil` Precommit message. If Tenderhellar is to
...
#### When does Tenderhellar call it?

When a validator _p_ is in Tenderhellar consensus state _prevote_ of round _r_, height _h_, in which _q_ is the proposer; and _p_ has received
...
then _p_&#39;s Tenderhellar locks _v_  and sends a Precommit message in the following way

1. _p_&#39;s Tenderhellar sets _lockedValue_ and _validValue_ to _v_, and sets _lockedRound_ and _validRound_ to _r_
2. _p_&#39;s Tenderhellar calls `RequestExtendVote` with _id(v)_ (`RequestExtendVote.hash`). The call is synchronous.
3. The Application optionally returns an array of bytes, `ResponseExtendVote.extension`, which is not interpreted by Tenderhellar.
4. _p_&#39;s Tenderhellar includes `ResponseExtendVote.extension` in a field of type [CanonicalVoteExtension](#canonicalvoteextension),
   it then populates the other fields in [CanonicalVoteExtension](#canonicalvoteextension), and signs the populated
   data structure.
5. _p_&#39;s Tenderhellar constructs and signs the [CanonicalVote](../core/data_structures.md#canonicalvote) structure.
6. _p_&#39;s Tenderhellar constructs the Precommit message (i.e. [Vote](../core/data_structures.md#vote) structure)
   using [CanonicalVoteExtension](#canonicalvoteextension) and [CanonicalVote](../core/data_structures.md#canonicalvote).
7. _p_&#39;s Tenderhellar broadcasts the Precommit message.

In the cases when _p_&#39;s Tenderhellar is to broadcast `precommit nil` messages (either _2f&#43;1_ `prevote nil` messages received,
or _timeoutPrevote_ triggered), _p_&#39;s Tenderhellar does **not** call `RequestExtendVote` and will not include
...
- The application must execute the transactions in full, in the order they appear in `RequestFinalizeBlock.txs`,
  before returning control to Tenderhellar. Alternatively, it can commit the candidate state corresponding to the same block
...
- Currently, Tenderhellar will fill up all fields in `RequestFinalizeBlock`, even if they were
...
Used to sync Tenderhellar with the application during a handshake that happens on startup.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| version | [string](#string) |  | The Tenderhellar software semantic version. |
| block_version | [uint64](#uint64) |  | The Tenderhellar Block Protocol version. |
| p2p_version | [uint64](#uint64) |  | The Tenderhellar P2P Protocol version. |
| abci_version | [string](#string) |  | The Tenderhellar ABCI semantic version. |
...
- This allows the app to decide if it wants to accept the initial validator set proposed by Tenderhellar
...
- OfferSnapshot is called when bootstrapping a node using state sync. The application may accept or reject snapshots
  as appropriate. Upon accepting, Tenderhellar will retrieve and apply snapshot chunks via ApplySnapshotChunk.
...
- `RequestPrepareProposal` contains a preliminary set of transactions `txs` that Tenderhellar considers to be a good block proposal, called _raw proposal_. The Application can modify this set via `ResponsePrepareProposal.tx_records` (see [TxRecord](#txrecord)).
    - The Application _can_ reorder, remove or add transactions to the raw proposal. Let `tx` be a transaction in `txs`:
        - If the Application considers that `tx` should not be proposed in this block, e.g., there are other transactions with higher priority, then it should not include it in `tx_records`. In this case, Tenderhellar won&#39;t remove `tx` from the mempool. The Application should be extra-careful, as abusing this feature may cause transactions to stay forever in the mempool.
        - If the Application considers that a `tx` should not be included in the proposal and removed from the mempool, then the Application should include it in `tx_records` and _mark_ it as `REMOVED`. In this case, Tenderhellar will remove `tx` from the mempool.
        - If the Application wants to add a new transaction, then the Application should include it in `tx_records` and _mark_ it as `ADD`. In this case, Tenderhellar will add it to the mempool.
    - The Application should be aware that removing and adding transactions may compromise _traceability_.
      &gt; Consider the following example: the Application transforms a client-submitted transaction `t1` into a second transaction `t2`, i.e., the Application asks Tenderhellar to remove `t1` and add `t2` to the mempool. If a client wants to eventually check what happened to `t1`, it will discover that `t_1` is not in the mempool or in a committed block, getting the wrong idea that `t_1` did not make it into a block. Note that `t_2` _will be_ in a committed block, but unless the Application tracks this information, no component will be aware of it. Thus, if the Application wants traceability, it is its responsability to support it. For instance, the Application could attach to a transformed transaction a list with the hashes of the transactions it derives from.
- Tenderhellar MAY include a list of transactions in `RequestPrepareProposal.txs` whose total size in bytes exceeds `RequestPrepareProposal.max_tx_bytes`.
...
- In same-block execution mode, the Application must provide values for `ResponsePrepareProposal.app_hash`,
  ...
  `ResponsePrepareProposal.consensus_param_updates`, as a result of fully executing the block.
    - The values for `ResponsePrepareProposal.validator_updates`, `ResponsePrepareProposal.core_chain_lock_update` or
      `ResponsePrepareProposal.consensus_param_updates` may be empty. In this case, Tenderhellar will keep
      the current values.
...
- As a result of executing the prepared proposal, the Application may produce header events or transaction events.
  The Application must keep those events until a block is decided and then pass them on to Tenderhellar via
  `ResponseFinalizeBlock`.
- As a sanity check, Tenderhellar will check the returned parameters for validity if the Application modified them.
...
- If Tenderhellar fails to validate the `ResponsePrepareProposal`, Tenderhellar will assume the application is faulty and crash.
    - The implementation of `PrepareProposal` can be non-deterministic.

#### When does Tenderhellar call it?

When a validator _p_ enters Tenderhellar consensus round _r_, height _h_, in which _p_ is the proposer,
and _p_&#39;s _validValue_ is `nil`:

1. _p_&#39;s Tenderhellar collects outstanding transactions from the mempool
    ...
    - _p_&#39;s Tenderhellar creates a block header.
2. _p_&#39;s Tenderhellar calls `RequestPrepareProposal` with the newly generated block.
   The call is synchronous: Tenderhellar&#39;s execution will block until the Application returns from the call.
...
5. _p_&#39;s Tenderhellar uses the (possibly) modified block as _p_&#39;s proposal in round _r_, height _h_.

Note that, if _p_ has a non-`nil` _validValue_, Tenderhellar will use it as proposal and will not call `RequestPrepareProposal`.
...
| local_last_commit | [CommitInfo](#tendermint-abci-CommitInfo) |  | Info about the last commit, obtained locally from Tenderhellar&#39;s data structures. |
| misbehavior | [Misbehavior](#tendermint-abci-Misbehavior) | repeated | List of information about validators that acted incorrectly. |
...
- The height and timestamp values match the values from the header of the proposed block.
- If `ResponseProcessProposal.status` is `REJECT`, Tenderhellar assumes the proposal received
  is not valid.
- In same-block execution mode, the Application is required to fully execute the block and provide values
  ...
  so that Tenderhellar can then verify the hashes in the block&#39;s header are correct.
  If the hashes mismatch, Tenderhellar will reject the block even if `ResponseProcessProposal.status`
  was set to `ACCEPT`.
...
When a validator _p_ enters Tenderhellar consensus round _r_, height _h_, in which _q_ is the proposer (possibly _p_ = _q_):

1. _p_ sets up timer `ProposeTimeout`.
2. If _p_ is the proposer, _p_ executes steps 1-6 in [PrepareProposal](#prepareproposal).
3. Upon reception of Proposal message (which contains the header) for round _r_, height _h_ from _q_, _p_&#39;s Tenderhellar verifies the block header.
4. Upon reception of Proposal message, along with all the block parts, for round _r_, height _h_ from _q_, _p_&#39;s Tenderhellar follows its algorithm
   to check whether it should prevote for the block just received, or `nil`
5. If Tenderhellar should prevote for the block just received
    1. Tenderhellar calls `RequestProcessProposal` with the block. The call is synchronous.
    ...
    3. If the returned value is
         - _accept_, Tenderhellar prevotes on this proposal for round _r_, height _h_.
         - _reject_, Tenderhellar prevotes `nil`.
...
- `RequestVerifyVoteExtension.vote_extension` can be an empty byte array. The Application&#39;s interpretation of it should be
  ...
  Tenderhellar will always call `RequestVerifyVoteExtension`, even for 0 length vote extensions.
- If `ResponseVerifyVoteExtension.status` is `REJECT`, Tenderhellar will reject the whole received vote.
...
When a validator _p_ is in Tenderhellar consensus round _r_, height _h_, state _prevote_ (**TODO** discuss: I think I must remove the state
from this condition, but not sure), and _p_ receives a Precommit message for round _r_, height _h_ from _q_:

1. If the Precommit message does not contain a vote extensions with a valid signature, Tenderhellar discards the message as invalid.
  - a 0-length vote extensions is valid as long as its accompanying signature is also valid.
2. Else, _p_&#39;s Tenderhellar calls `RequestVerifyVoteExtension`.
3. The Application returns _accept_ or _reject_ via `ResponseVerifyVoteExtension.status`.
4. If the Application returns
  - _accept_, _p_&#39;s Tenderhellar will keep the received vote, together with its corresponding
  ...
  - _reject_, _p_&#39;s Tenderhellar will deem the Precommit message invalid and discard it.
...
| round | [int32](#int32) |  | Round number for the block. |
| vote_extensions | [ExtendVoteExtension](#tendermint-abci-ExtendVoteExtension) | repeated | Application-specific information signed by Tenderhellar. Can have 0 length. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| vote_extensions | [ExtendVoteExtension](#tendermint-abci-ExtendVoteExtension) | repeated | Optional information signed by Tenderhellar. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| action | [TxRecord.TxAction](#tendermint-abci-TxRecord-TxAction) |  | What should Tenderhellar do with this transaction? |
