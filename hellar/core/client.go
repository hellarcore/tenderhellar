...
type Client interface {
	...
	// Close Closes connection to hellard
	Close() error
	// Ping Sends ping to hellard
	Ping() error
}

// RPCClient implements Client
// Handles connection to the underlying hellard instance
...
func NewRPCClient(host string, username string, password string, logger log.Logger) (*RPCClient, error) {
	if host == "" {
		return nil, fmt.Errorf("unable to establish connection to the Hellar Core node")
	}
	...
	hellarCoreClient := RPCClient{
		endpoint: client,
		logger:   logger,
	}

	return &hellarCoreClient, nil
}
