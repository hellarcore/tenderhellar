...
func TestNewValidator(t *testing.T) {
	...
	validator := types.NewValidator(pubKey, types.DefaultHellarVotingPower, proTxHash,
		fmt.Sprintf("tcp://%s@127.0.0.1:12345", nodeID))
	require.NotNil(t, validator)
	newNodeID := validator.NodeAddress.NodeID
	assert.Equal(t, nodeID, newNodeID)

	validator = types.NewValidator(pubKey, types.DefaultHellarVotingPower, proTxHash, "127.0.0.1:23456")
	require.NotNil(t, validator)
