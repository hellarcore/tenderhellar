...
import (
	...
	"github.com/hellarcore/tenderhellar/hellar/quorum/mock"
	"github.com/hellarcore/tenderhellar/hellar/quorum/selectpeers"
	"github.com/hellarcore/tenderhellar/internal/eventbus"
	"github.com/hellarcore/tenderhellar/internal/mempool/mocks"
...
// and ensures operations executed on mock Hellar Connection Manager (history records)
...
func setup(
	...
	t *testing.T,
	me *types.Validator,
) (eventBus *eventbus.EventBus, sw *mock.HellarDialer, vc *ValidatorConnExecutor) {
	logger := log.NewTestingLogger(t)
	eventBus = eventbus.NewDefault(logger)
...
func cleanup(_t *testing.T, bus *eventbus.EventBus, _dialer p2p.HellarDialer, vc *ValidatorConnExecutor) {
	bus.Stop()
	vc.Stop()
}
