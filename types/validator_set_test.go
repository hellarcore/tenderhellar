...
import (
	...
	tmmath "github.com/hellarcore/tenderhellar/libs/math"
...
func randModuloValidator(totalVotingPower int64) *Validator {
	...
	val := NewValidator(randPubKey(), DefaultHellarVotingPower, crypto.RandProTxHash(), address)
...
func randValidatorInQuorum(ctx context.Context, t *testing.T, quorumHash crypto.QuorumHash) (*Validator, PrivValidator) {
	...
	address := RandValidatorAddress().String()
	val := NewValidator(pubKey, DefaultHellarVotingPower, proTxHash, address)
	return val, privVal
}
...
func testValSet(nVals int) []testVal {
	...
	for i := 0; i < nVals; i++ {
		vals[i] = testVal{fmt.Sprintf("v%d", i+1), DefaultHellarVotingPower}
	}
	return vals
}
...
func TestValSetUpdatesDuplicateEntries(t *testing.T) {
	testCases := []valSetErrTestCaseWithErr{
		...
		{ // first entry is duplicated change
			testValSet(2),
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			"duplicate entry Validator",
		},
		{ // second entry is duplicated change
			testValSet(2),
			[]testVal{{"v2", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
			"duplicate entry Validator",
		},
		{ // change duplicates are separated by a valid change
			testValSet(2),
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			"duplicate entry Validator",
		},
		{ // change duplicates are separated by a valid change
			testValSet(3),
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			"duplicate entry Validator",
		},
		...
		{ // remove and update same val
			testValSet(2),
			[]testVal{{"v1", 0}, {"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			"duplicate entry Validator",
		},
		{ // duplicate entries in removes + changes
			testValSet(2),
			[]testVal{{"v1", 0}, {"v2", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v1", 0}},
			"duplicate entry Validator",
		},
		{ // duplicate entries in removes + changes
			testValSet(3),
			[]testVal{{"v1", 0}, {"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v1", 0}},
			"duplicate entry Validator",
		},
	}
...
func TestValSetUpdatesOtherErrors(t *testing.T) {
	testCases := []valSetErrTestCase{
		...
		{ // delete all validators
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}},
...
func TestValSetUpdatesBasicTestsExecute(t *testing.T) {
	valSetUpdatesBasicTests := []struct {
		...
		{ // voting power changes
			testValSet(2),
			[]testVal{{"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
		},
		{ // add new validators
			[]testVal{{"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			[]testVal{{"v4", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v4", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
		},
		{ // add new validator to middle
			[]testVal{{"v3", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			[]testVal{{"v2", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
		},
		{ // add new validator to beginning
			[]testVal{{"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
		},
		{ // delete validators
			[]testVal{{"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			[]testVal{{"v2", 0}},
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}},
		},
	}
...
func TestValSetUpdatesOrderIndependenceTestsExecute(t *testing.T) {
	...
	valSetUpdatesOrderTests := []struct {
		...
		0: { // order of changes should not matter, the final validator sets should be the same
			[]testVal{{"v4", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			[]testVal{{"v4", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}}},

		1: { // order of additions should not matter
			[]testVal{{"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			[]testVal{{"v3", DefaultHellarVotingPower}, {"v4", DefaultHellarVotingPower}, {"v5", DefaultHellarVotingPower}, {"v6", DefaultHellarVotingPower}}},
...
func TestValSetApplyUpdatesTestsExecute(t *testing.T) {
	...
	valSetUpdatesBasicTests := []struct {
		...
		// additions
		0: { // prepend
			[]testVal{{"v4", DefaultHellarVotingPower}, {"v5", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v4", DefaultHellarVotingPower}, {"v5", DefaultHellarVotingPower}}},
		1: { // append
			[]testVal{{"v4", DefaultHellarVotingPower}, {"v5", DefaultHellarVotingPower}},
			[]testVal{{"v6", DefaultHellarVotingPower}},
			[]testVal{{"v6", DefaultHellarVotingPower}, {"v4", DefaultHellarVotingPower}, {"v5", DefaultHellarVotingPower}}},
		2: { // insert
			[]testVal{{"v4", DefaultHellarVotingPower}, {"v6", DefaultHellarVotingPower}},
			[]testVal{{"v5", DefaultHellarVotingPower}},
			[]testVal{{"v6", DefaultHellarVotingPower}, {"v4", DefaultHellarVotingPower}, {"v5", DefaultHellarVotingPower}}},
		3: { // insert multi
			[]testVal{{"v4", DefaultHellarVotingPower}, {"v6", DefaultHellarVotingPower}, {"v9", DefaultHellarVotingPower}},
			[]testVal{{"v5", DefaultHellarVotingPower}, {"v7", DefaultHellarVotingPower}, {"v8", DefaultHellarVotingPower}},
			[]testVal{{"v8", DefaultHellarVotingPower}, {"v7", DefaultHellarVotingPower}, {"v6", DefaultHellarVotingPower}, {"v9", DefaultHellarVotingPower}, {"v4", DefaultHellarVotingPower}, {"v5", DefaultHellarVotingPower}}},
		// changes
		4: { // head
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}},
		...
		8: {
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}, {"v4", DefaultHellarVotingPower}},
			[]testVal{{"v1", DefaultHellarVotingPower}, {"v4", DefaultHellarVotingPower}, {"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}}},
	}
...
func randTestVSetCfg(t *testing.T, nBase, nAddMax int) testVSetCfg {
	...
	for i := 0; i < nBase; i++ {
		cfg.startVals[i] = testVal{fmt.Sprintf("v%d", i), DefaultHellarVotingPower}
		if i < nOld {
			cfg.expectedVals[i] = cfg.startVals[i]
		}
		if i >= nOld && i < nOld+nChanged {
			cfg.updatedVals[i-nOld] = testVal{fmt.Sprintf("v%d", i), DefaultHellarVotingPower}
	...
	for i := nBase; i < nBase+nAdd; i++ {
		cfg.addedVals[i-nBase] = testVal{fmt.Sprintf("v%d", i), DefaultHellarVotingPower}
...
func TestValSetUpdatePriorityOrderTests(t *testing.T) {
	...
	testCases := []testVSetCfg{
		0: { // remove high power validator, keep old equal lower power validators
			startVals:    []testVal{{"v3", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
			deletedVals:  []testVal{{"v3", 0}},
			updatedVals:  []testVal{},
			addedVals:    []testVal{},
			expectedVals: []testVal{{"v1", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
		},
		1: { // remove high power validator, keep old different power validators
			startVals:    []testVal{{"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			deletedVals:  []testVal{{"v3", 0}},
			updatedVals:  []testVal{},
			addedVals:    []testVal{},
			expectedVals: []testVal{{"v1", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
		...
		2: { // remove high power validator, add new low power validators, keep old lower power
			startVals:    []testVal{{"v3", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}, {"v1", DefaultHellarVotingPower}},
			deletedVals:  []testVal{{"v3", 0}},
			updatedVals:  []testVal{{"v2", DefaultHellarVotingPower}},
			addedVals:    []testVal{{"v4", DefaultHellarVotingPower}, {"v5", DefaultHellarVotingPower}},
			expectedVals: []testVal{{"v1", DefaultHellarVotingPower}, {"v4", DefaultHellarVotingPower}, {"v5", DefaultHellarVotingPower}, {"v2", DefaultHellarVotingPower}},
		},
