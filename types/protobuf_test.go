...
func TestABCIValidatorFromPubKeyAndPower(t *testing.T) {
	...
	abciVal := TM2PB.NewValidatorUpdate(pubkey, DefaultHellarVotingPower, crypto.RandProTxHash(), address.String())
	assert.Equal(t, DefaultHellarVotingPower, abciVal.Power)
	assert.Equal(t, address.String(), abciVal.NodeAddress)

	assert.NotPanics(t, func() {
		TM2PB.NewValidatorUpdate(nil, DefaultHellarVotingPower, crypto.RandProTxHash(), RandValidatorAddress().String())
	})
...
func TestABCIValidatorWithoutPubKey(t *testing.T) {
	...
	tmValExpected := abci.Validator{
		Power:     DefaultHellarVotingPower,
		ProTxHash: proTxHash,
	}
