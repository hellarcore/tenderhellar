...
import (
	...
	"github.com/hellarcore/tenderhellar/internal/p2p/p2ptest"
	tmpubsub "github.com/hellarcore/tenderhellar/internal/pubsub"
	sm "github.com/hellarcore/tenderhellar/internal/state"
	statemocks "github.com/hellarcore/tenderhellar/internal/state/mocks"
...
func (rts *reactorTestSuite) switchToConsensus(ctx context.Context) {
	for nodeID, reactor := range rts.reactors {
		...
		sCtx := hellar.ContextWithProTxHash(ctx, rts.states[nodeID].privValidator.ProTxHash)
...
func setup(
	...
	for i := 0; i < numNodes; i++ {
		state := states[i]
		sCtx := hellar.ContextWithProTxHash(ctx, states[i].privValidator.ProTxHash)
