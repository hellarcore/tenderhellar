...
import (
	...
	tmpubsub "github.com/hellarcore/tenderhellar/internal/pubsub"
	tmquery "github.com/hellarcore/tenderhellar/internal/pubsub/query"
	...
	tmevents "github.com/hellarcore/tenderhellar/libs/eventemitter"
...
func TestStateEnterProposeYesPrivValidator(t *testing.T) {
	...
	cs, _ := makeState(ctx, t, makeStateArgs{config: config, validators: 1})
	ctx = hellar.ContextWithProTxHash(ctx, cs.privValidator.ProTxHash)
	stateData := cs.GetStateData()
	height, round := stateData.Height, stateData.Round
...
func TestStateProposalTime(t *testing.T) {
	...
	ctx = hellar.ContextWithProTxHash(ctx, cs1.privValidator.ProTxHash)
	stateData := cs1.GetStateData()
	height, round := stateData.Height, stateData.Round
...
func TestStateTryAddCommitCallsProcessProposal(t *testing.T) {
	...
	ctx = hellar.ContextWithProTxHash(ctx, otherNode.privValidator.ProTxHash)
