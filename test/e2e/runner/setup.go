...
import (
	...
	cryptoenc "github.com/hellarcore/tenderhellar/crypto/encoding"
	...
	"github.com/hellarcore/tenderhellar/test/e2e/pkg/infra"
...
const (
	...
	PrivvalAddressHellarCore = "127.0.0.1:19998"
...
func Setup(ctx context.Context, logger log.Logger, testnet *e2e.Testnet, ti infra.TestnetInfra) error {
	...
	for _, node := range testnet.Nodes {
		...
		// Set up a dummy validator. Tenderhellar requires a file PV even when not used, so we
...
func MakeGenesis(testnet *e2e.Testnet, genesisTime time.Time) (types.GenesisDoc, error) {
	...
	for validator, validatorUpdate := range testnet.Validators {
		...
		genesis.Validators = append(genesis.Validators, types.GenesisValidator{
			...
			Power:     types.DefaultHellarVotingPower,
		})
	}
	// The validator set will be sorted internally by Tenderhellar ranked by power,
...
// MakeConfig generates a Tenderhellar config for a node.
func MakeConfig(node *e2e.Node) (*config.Config, error) {
	...
	// Tenderhellar errors if it does not have a privval key set up, regardless of whether
	...
	if node.PrivvalProtocol == e2e.ProtocolHellarCore {
...
func MakeAppConfig(node *e2e.Node) ([]byte, error) {
	cfg := map[string]interface{}{
		...
		"privval_server_type":       "hellarcore",
		"privval_server":            PrivvalAddressHellarCore,
	...
	if node.Mode == e2e.ModeValidator {
		...
		case e2e.ProtocolHellarCore:
			cfg["privval_server_type"] = "hellarcore"
			cfg["privval_server"] = PrivvalAddressHellarCore
		case e2e.ProtocolUNIX:
	...
	} else if node.PrivvalProtocol == e2e.ProtocolHellarCore {
		cfg["privval_server_type"] = "hellarcore"
		cfg["privval_server"] = PrivvalAddressHellarCore
	}
