...
func TestServer(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			e:     "hellar is the best coin",
			query: url.Values{},
		},
		{
			url: "http://localhost:9981/test?q1=100&q2=bc",
			e:   "hellar is the best ever coin",
			query: url.Values{
				"q1": []string{"100"},
				"q2": []string{"bc"},
			},
...
func TestHellarCoreSignerPingMethod(t *testing.T) {
	...
	logger := log.NewTestingLogger(t)
	hellarCoreRPCClient, err := hellarcore.NewRPCClient(addr, "root", "root", logger)
	assert.NoError(t, err)
	client, err := privval.NewHellarCoreSignerClient(hellarCoreRPCClient, btcjson.LLMQType_5_60, logger)
	assert.NoError(t, err)
	err = client.Ping()
	assert.NoError(t, err)
	srv.Stop(ctx)
