...
import (
	...
	"github.com/hellarcore/tenderhellar/abci/server"
	"github.com/hellarcore/tenderhellar/config"
	"github.com/hellarcore/tenderhellar/crypto/ed25519"
	...
	lproxy "github.com/hellarcore/tenderhellar/light/proxy"
	lrpc "github.com/hellarcore/tenderhellar/light/rpc"
	...
	grpcprivval "github.com/hellarcore/tenderhellar/privval/grpc"
	privvalproto "github.com/hellarcore/tenderhellar/proto/tendermint/privval"
	rpcserver "github.com/hellarcore/tenderhellar/rpc/jsonrpc/server"
	"github.com/hellarcore/tenderhellar/test/e2e/app"
	e2e "github.com/hellarcore/tenderhellar/test/e2e/pkg"
	"github.com/hellarcore/tenderhellar/test/e2e/pkg/mockcoreserver"
)
...
func startMockCoreSrv(cfg *Config, logger log.Logger) error {
	...
	hellarCoreRPCClient, err = hellarcore.NewRPCClient(
		cfg.PrivValServer,
		tmcfg.PrivValidator.CoreRPCUsername,
		tmcfg.PrivValidator.CoreRPCPassword,
		logger.With("module", hellarcore.ModuleName),
	)
	if err != nil {
		return fmt.Errorf("connection to Hellar Core RPC failed: %w", err)
	}
	return nil
}

func startRemoteSigner(ctx context.Context, cfg *Config, logger log.Logger) error {
	if cfg.PrivValServerType == "hellarcore" {
...
// startNode starts a Tenderhellar node running the application directly. It assumes the Tenderhellar
// configuration is in $TMHOME/config/tenderhellar.toml.
//
