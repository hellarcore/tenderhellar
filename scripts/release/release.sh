...
This script prepares new release of Tenderhellar. It:
...
where flags can be one of:
    ...
    -C=<path> - path to local Tenderhellar repository
...
function createReleasePR {
    ...
    gh api --silent --method POST 'repos/hellarpro/tenderhellar/milestones' --field "title=${MILESTONE}" || true
