...
import (
	...
	"github.com/hellarcore/tenderhellar/internal/p2p/client"
	"github.com/hellarcore/tenderhellar/internal/p2p/conn"
	...
	tmnet "github.com/hellarcore/tenderhellar/libs/net"
	"github.com/hellarcore/tenderhellar/libs/service"
	"github.com/hellarcore/tenderhellar/privval"
	tmgrpc "github.com/hellarcore/tenderhellar/privval/grpc"
...
func createPrivval(ctx context.Context, logger log.Logger, conf *config.Config, genDoc *types.GenesisDoc) (types.PrivValidator, error) {
	...
	case conf.PrivValidator.CoreRPCHost != "": // HELLAR Core Privval
		if conf.Mode != config.ModeValidator {
			return nil, fmt.Errorf("cannot initialize PrivValidator: this node is NOT a validator")
		}

		logger.Info("Initializing Hellar Core PrivValidator")

		hellarCoreRPCClient, err := DefaultHellarCoreRPCClient(conf, logger.With("module", hellarcore.ModuleName))
		if err != nil {
			return nil, fmt.Errorf("failed to create Hellar Core RPC client: %w", err)
		}

		logger.Info("Waiting for Hellar Core RPC Client to be ready")
		err = hellarcore.WaitForMNReady(hellarCoreRPCClient, time.Second)
		if err != nil {
			return nil, fmt.Errorf("failed to wait for masternode status 'ready': %w", err)
		}
		logger.Info("Hellar Core RPC Client is ready")

		// If a local port is provided for Hellar Core rpc into the service to sign.
		privValidator, err := createAndStartPrivValidatorHellarCoreClient(
			genDoc.QuorumType,
			hellarCoreRPCClient,
			logger,
		)
		...
		if err != nil {
			return nil, fmt.Errorf("can't get proTxHash using hellar core signing: %w", err)
		}
...
func createAndStartPrivValidatorHellarCoreClient(
	defaultQuorumType btcjson.LLMQType,
	hellarCoreRPCClient hellarcore.Client,
	logger log.Logger,
) (types.PrivValidator, error) {
	pvsc, err := privval.NewHellarCoreSignerClient(hellarCoreRPCClient, defaultQuorumType, logger)
	if err != nil {
		return nil, fmt.Errorf("failed to start private validator: %w", err)
	}
