...
import (
	...
	"github.com/hellarcore/tenderhellar/hellar"
	"github.com/hellarcore/tenderhellar/hellar/core"
	hellarquorum "github.com/hellarcore/tenderhellar/hellar/quorum"
	"github.com/hellarcore/tenderhellar/internal/blocksync"
	"github.com/hellarcore/tenderhellar/internal/consensus"
	"github.com/hellarcore/tenderhellar/internal/eventbus"
	"github.com/hellarcore/tenderhellar/internal/eventlog"
	...
	p2pclient "github.com/hellarcore/tenderhellar/internal/p2p/client"
	"github.com/hellarcore/tenderhellar/internal/p2p/pex"
	"github.com/hellarcore/tenderhellar/internal/proxy"
	rpccore "github.com/hellarcore/tenderhellar/internal/rpc/core"
	sm "github.com/hellarcore/tenderhellar/internal/state"
	"github.com/hellarcore/tenderhellar/internal/state/indexer"
	"github.com/hellarcore/tenderhellar/internal/state/indexer/sink"
	"github.com/hellarcore/tenderhellar/internal/statesync"
	"github.com/hellarcore/tenderhellar/internal/store"
...
type nodeImpl struct {
	...
	// Hellar
	validatorConnExecutor *hellarquorum.ValidatorConnExecutor
}
...
func makeNode(
	...
	var (
		...
		hellarCoreRPCClient core.Client
	)
	switch cfg.Mode {
	case config.ModeValidator:
		...
		if hellarPrivval, ok := privValidator.(privval.HellarPrivValidator); ok {
			hellarCoreRPCClient = hellarPrivval.HellarRPCClient()
		}
	...
	case config.ModeFull:
		...
		if cfg.PrivValidator.CoreRPCHost != "" {
			hellarCoreRPCClient, err = DefaultHellarCoreRPCClient(cfg, logger.With("module", core.ModuleName))
			if err != nil {
				return nil, fmt.Errorf("failed to create Hellar Core RPC client: %w", err)
			}
		} else {
			...
			hellarCoreRPCClient = core.NewMockClient(cfg.ChainID(), llmqType, privValidator, false)
		}
	}
	...
	// Start Hellar connection executor
	var validatorConnExecutor *hellarquorum.ValidatorConnExecutor
	if len(proTxHash) > 0 {
		vcLogger := logger.With("node_proTxHash", proTxHash.ShortString(), "module", "ValidatorConnExecutor")
		dcm := p2p.NewRouterHellarDialer(peerManager, vcLogger)
		validatorConnExecutor, err = hellarquorum.NewValidatorConnExecutor(
			proTxHash,
			eventBus,
			dcm,
			hellarquorum.WithLogger(vcLogger),
			hellarquorum.WithValidatorsSet(state.Validators),
		)
		if err != nil {
			return nil, combineCloseError(err, makeCloser(closers))
		}
	...
	// https://github.com/hellarcore/tenderhellar/issues/4644
...
func (n *nodeImpl) OnStart(ctx context.Context) error {
	...
	ctx = hellar.ContextWithProTxHash(ctx, proTxHash)
...
// DefaultHellarCoreRPCClient returns RPC client for the Hellar Core node
func DefaultHellarCoreRPCClient(cfg *config.Config, logger log.Logger) (core.Client, error) {
