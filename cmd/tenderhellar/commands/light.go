...
func MakeLightCommand(conf *config.Config, logger log.Logger) *cobra.Command {
	var (
		...
		hellarCoreRPCHost string
		hellarCoreRPCUser string
		hellarCoreRPCPass string
	)
...
`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			options := []light.Option{
				light.Logger(logger),
				light.HellarCoreVerification(),
			}

			rpcLogger := logger.With("module", hellarcore.ModuleName)
			hellarCoreRPCClient, _ := hellarcore.NewRPCClient(hellarCoreRPCHost, hellarCoreRPCUser, hellarCoreRPCPass, rpcLogger)
	...
	cmd.Flags().StringVar(&hellarCoreRPCHost, "dchost", "",
		"host address of the Hellar Core RPC node")
	cmd.Flags().StringVar(&hellarCoreRPCHost, "dcuser", "",
		"Hellar Core RPC node user")
	cmd.Flags().StringVar(&hellarCoreRPCHost, "dcpass", "",
		"Hellar Core RPC node password")
