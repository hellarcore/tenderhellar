...
OUTPUT ?= build/tenderhellar
BUILDDIR ?= $(CURDIR)/build
REPO_NAME ?= github.com/hellarpro/tenderhellar
BUILD_TAGS ?= tenderhellar
...
-L$(BLS_DIR)/build/src \
-lhellarbls -lrelic_s -lmimalloc-secure -lgmp
...
build-binary:
	$(GO) build $(BUILD_FLAGS) -tags '$(BUILD_TAGS)' -o $(OUTPUT) ./cmd/tenderhellar/
.PHONY: build-binary

install:
	$(GO) install $(BUILD_FLAGS) -tags $(BUILD_TAGS) ./cmd/tenderhellar
.PHONY: install
...
build-docker:
	docker buildx build \
		--load \
		--cache-from=type=registry,ref=hellarpay/tenderhellar:buildcache-deps \
		--label=tendermint \
		--tag="hellarpay/tenderhellar" \
		-f DOCKER/Dockerfile .
.PHONY: build-docker
...
build_c-amazonlinux:
	...
	docker run --rm -it -v `pwd`:/tendermint hellarpay/tenderhellar:build_c-amazonlinux
.PHONY: build_c-amazonlinux

# Run a 4-node testnet locally
localnet-start: localnet-stop build-docker-localnode
	@if ! [ -f build/node0/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/tendermint:Z hellarpay/tenderhellar testnet --config /etc/tendermint/config-template.toml --o . --starting-ip-address 192.167.10.2; fi
	docker-compose up
