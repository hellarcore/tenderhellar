...
const (
	hellarCoreVerification mode = iota + 1
...
// HellarCoreVerification option configures the light client to go to the last block and verify it was signed by the
// Quorum that is has in quorum hash.
func HellarCoreVerification() Option {
	return func(c *Client) {
		c.verificationMode = hellarCoreVerification
	}
}
...
type Client struct {
	...
	// Rpc client connected to hellard
...
func NewClient(
	...
	return NewClientAtHeight(ctx, 0, chainID, primary, witnesses, trustedStore, hellarCoreRPCClient, options...)
}
...
func NewClientAtHeight(
	...
	c, err := NewClientFromTrustedStore(chainID, primary, witnesses, trustedStore, hellarCoreRPCClient, options...)
	if err != nil {
...
func NewClientFromTrustedStore(
	...
	if hellarCoreRPCClient == nil {
		return nil, ErrNoHellarCoreClient
	}
	...
	c := &Client{
		chainID:           chainID,
		verificationMode:  hellarCoreVerification,
		...
		hellarCoreRPCClient: hellarCoreRPCClient,
	}
...
func (c *Client) initializeAtHeight(ctx context.Context, height int64) error {
	...
	// 4) Ensure that the commit is valid based on local hellar core verification.
	err = c.verifyBlockWithHellarCore(ctx, l)
	if err != nil {
...
func (c *Client) verifyLightBlock(ctx context.Context, newLightBlock *types.LightBlock, _now time.Time) error {
	...
	case hellarCoreVerification:
		verifyFunc = c.verifyBlockWithHellarCore
	default:
...
// verifyBlockWithHellarCore verifies block and StateID signature using Hellar Core RPC service
// This method is called from verifyLightBlock if verification mode is hellarcore,
// verifyLightBlock in its turn is called by VerifyHeader.
func (c *Client) verifyBlockWithHellarCore(ctx context.Context, newLightBlock *types.LightBlock) error {
	if err := c.verifyBlockSignatureWithHellarCore(ctx, newLightBlock); err != nil {
		return err
	}
	return nil
}

func (c *Client) verifyBlockSignatureWithHellarCore(_ctx context.Context, newLightBlock *types.LightBlock) error {
	...
	blockSignatureIsValid, err := c.hellarCoreRPCClient.QuorumVerify(
		quorumType,
		blockRequestID,
		blockMessageHash,
